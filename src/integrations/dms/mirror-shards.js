const APP_VERSION = '20260629-pigment-frosted-glass-v1'
...
const BUTTON_RANDOM_MIN = 0.96
const BUTTON_RANDOM_MAX = 1.24
...
const DMS_GPU_MOTION_GLSL = `
  ...
  void dmsAccumulateHit(
    ...
    if (localAge > 0.0) {
      ...
      float instant = cos(age * 34.0 + dmsSeed * 7.0) *
        ...
        spatialFalloff *
        dmsHitActive;
      globalWave += instant * 0.44;
      localKick += instant * 0.32;
      localSpin += instant * 0.22;
    }
  }
  ...
  vec3 dmsApplyMotion(vec3 basePosition) {
    ...
    float localSpin = 0.0;
    dmsAccumulateHit(uDmsHit0, uDmsHitPanels.x, phase, globalWave, localKick, localSlide, localSpin);
    dmsAccumulateHit(uDmsHit1, uDmsHitPanels.y, phase, globalWave, localKick, localSlide, localSpin);
    dmsAccumulateHit(uDmsHit2, uDmsHitPanels.z, phase, globalWave, localKick, localSlide, localSpin);
    ...
    float normalOffset = globalWave * 0.055 + localKick * 0.030 + coherentBreath + idle;
    vec2 tangent = vec2(cos(phase), sin(phase));

    vec3 moved = basePosition;
    float rx = globalWave * 0.035 + localSpin * 0.105 + coherentBreath * 0.18;
    float ry = globalWave * 0.018 + localSpin * 0.045 + sin(uDmsTime * 0.64 + dmsSeed * 18.0) * 0.005;
    float rz = localSpin * 0.022 + sin(uDmsTime * 0.52 + dmsSeed * 9.0) * 0.0025;

    moved.yz = dmsRotate2(moved.yz, rx);
    moved.xz = dmsRotate2(moved.xz, ry);
    moved.xy = dmsRotate2(moved.xy, rz);
    moved.xy += tangent * localSlide * 0.018;
...
const DMS_LIQUID_FRAGMENT_SHADER = `
  ...
  vec3 palette(float x) {
    ...
    vec3 cyan = vec3(0.04, 0.66, 1.0);
    vec3 cobalt = vec3(0.05, 0.28, 0.96);
    vec3 ultramarine = vec3(0.02, 0.10, 0.58);
    vec3 col = mix(paper, ice, smoothstep(0.08, 0.34, x));
    col = mix(col, cyan, smoothstep(0.28, 0.52, x) * 0.72);
    col = mix(col, cobalt, smoothstep(0.45, 0.78, x));
    col = mix(col, ultramarine, smoothstep(0.74, 0.96, x) * 0.72);
    return col;
  }
  ...
  void main() {
    ...
    float t = uTime * 0.115;
    float finalGuide = smoothstep(0.08, 1.0, uFinal) * uTargetReady;
    vec2 texel = 1.0 / max(uResolution, vec2(1.0));
    vec2 targetDrift = finalGuide * 0.018 * vec2(
      fbm(uv * 3.2 + vec2(t * 1.7, -t * 1.1)),
      fbm(uv * 3.0 + vec2(-t * 1.2, t * 1.6) + 5.0)
    );
    vec2 refUv = clamp(uv + targetDrift - finalGuide * 0.009, 0.0, 1.0);
    ...
    vec2 p = uv - 0.5;
    p.x *= aspect;
    float choice = uChoice * 0.28;
    float attraction = smoothstep(0.02, 0.92, uFinal) * uTargetReady;
    ...
    pointerP.x *= aspect;
    float pointerFalloff = exp(-dot(p - pointerP, p - pointerP) / 0.032);
    p = swirl(p, pointerP, uRipple * (1.2 + uFinal * 0.4), 0.044);
    p += uRipple * pointerFalloff * 0.030 * vec2(
      sin(t * 18.0 + p.y * 22.0),
      cos(t * 16.0 + p.x * 20.0)
    );

    p = swirl(p, vec2(-0.18 + choice, 0.08), 2.2 + uPulse * 0.18, 0.18);
    p = swirl(p, vec2(0.24 - choice * 0.4, -0.24), -1.75, 0.13);
    ...
    for (int i = 0; i < 16; i++) {
      float fi = float(i);
      vec2 c = vec2(
        0.10 + 0.80 * hash(vec2(fi * 2.11, 4.7)),
        0.08 + 0.84 * hash(vec2(9.1, fi * 3.03))
      );
      c += 0.030 * vec2(
        sin(t * (2.0 + fi * 0.07) + fi * 1.8),
        cos(t * (1.5 + fi * 0.04) + fi * 1.3)
      );
      float rr = 0.020 + 0.052 * hash(vec2(fi * 5.2, 1.6));
      float local = 0.55 + 0.45 * smoothstep(0.1, 0.95, fbm((uv - c) * 5.0 + fi));
      cells += cellRing(uv, c, rr, aspect) * local;
      fills += cellFill(uv, c, rr * 0.82, aspect) * (0.42 + 0.44 * hash(vec2(fi, 8.0)));
    }
    ...
    float morph = finalGuide * organicReveal;
    ...
    float pigmentEdge = clamp(refEdge * 0.72 + flowingEdge * 0.58 + darkVein * 0.28, 0.0, 1.0);
    ...
    float powderFlow = smoothstep(0.62, 0.98, fbm(q * vec2(72.0, 96.0) + refGradient * 9.0));
    ...
    vec3 color = palette(unifiedDensity);
    vec3 paleAcrylic = mix(vec3(0.965, 0.992, 1.0), vec3(0.58, 0.84, 1.0), visibleBroad * 0.86);
    color = mix(color, paleAcrylic, paperPool * (0.30 + morph * 0.12));
    vec3 cellBlue = mix(vec3(0.66, 0.91, 1.0), vec3(0.02, 0.25, 0.95), unifiedDensity);
    color = mix(color, cellBlue, cells * (0.62 - morph * 0.24));
    color = mix(color, vec3(0.0, 0.16, 0.80), darkVein * (0.46 + morph * 0.14));
    color = mix(color, vec3(0.0, 0.28, 1.0), visibleEdge * (0.10 + morph * 0.12) + visibleDetail * 0.035);
    color = mix(color, vec3(0.965, 0.992, 1.0), (whiteVein * 0.28 + fills * 0.10 + foam * 0.05) * (0.56 + morph * 0.08));
    float paleBlueLift = (1.0 - smoothstep(0.22, 0.78, unifiedDensity)) *
      (0.50 + 0.50 * smoothstep(0.02, 0.56, body + cells + visibleBroad));
    color = mix(color, vec3(0.58, 0.86, 1.0), paleBlueLift * 0.18);
    color = mix(color, vec3(0.74, 0.93, 1.0), frostedFilm * 0.10);
    color = mix(color, vec3(0.20, 0.58, 1.0), palePowder * 0.34);
    color = mix(color, vec3(0.00, 0.20, 0.96), edgePowder * 0.54 + deepSediment * 0.38);
    color = mix(color, vec3(0.00, 0.10, 0.72), deepSediment * 0.28);
    color += (grain - 0.5) * (0.020 + frostMask * 0.030);
    color = mix(vec3(0.940, 0.982, 1.0), color, clamp(0.80 + vignette * 0.20, 0.0, 1.0));
    ...
    float paintAlpha = mix(0.56, 0.88, smoothstep(0.08, 0.72, bluePresence));
...
const SHARD_VARIANT_URLS = {
  ...
  diagonal_shear: `${DMS_ROOT}assets/shards-diagonal-shear.json`,
  asymmetric_web: `${DMS_ROOT}assets/shards-asymmetric-web.json`,
}
...
function colorForShard(THREE, seed) {
  ...
  color.setHSL(0.52 + hueShift, 0.34, 0.78 + stableNoise(seed, 5) * 0.08)
  return color
}
...
function createGpuMotionUniforms(THREE) {
  return {
  ...
  material.onBeforeCompile = function onBeforeCompile(shader, renderer) {
    ...
    if (shader.fragmentShader.includes('#include <output_fragment>')) {
      shader.fragmentShader = shader.fragmentShader.replace('#include <output_fragment>', `
        ...
        outgoingLight += vec3(0.48, 0.86, 1.0) * dmsFresnel * 0.42;
        outgoingLight += vec3(1.0, 1.0, 1.0) * dmsGrazingLine * 0.28;
        outgoingLight += vec3(0.90, 0.98, 1.0) * 0.035;
        diffuseColor.a *= 0.62 + dmsFresnel * 0.34;
...
function updateGpuMotionUniforms(group, interactions, timeSeconds) {
  const uniforms = group?.userData?.gpuMotionUniforms
  if (!uniforms) return
...
function createLiquidPosterUniforms(THREE, texture) {
...
function createLiquidPosterMaterial(THREE, texture, motionUniforms = null) {
  const uniforms = {
    ...createLiquidPosterUniforms(THREE, texture),
    ...(motionUniforms || {}),
  }
...
function updateLiquidPosterEffect(group, timeSeconds) {
  ...
  state.final += (state.targetFinal - state.final) * 0.024
  state.choice += (state.targetChoice - state.choice) * 0.035
  state.pulse *= 0.965
  state.ripple *= 0.91
  ...
  for (const uniforms of uniformsList) {
    ...
    uniforms.uChoice.value = state.choice
...
function driveLiquidPosterEffect(group, x, y, strength, panelIndex, mode = 'pulse') {
  const state = group?.userData?.liquidPosterState
  if (!state) return
  ...
  state.pointerX = uv.x
  state.pointerY = uv.y
  state.ripple = Math.min(1.4, state.ripple + 0.42 * normalizedStrength)
  state.pulse = Math.max(state.pulse, 0.28 * normalizedStrength)
  state.targetChoice = clamp(state.targetChoice * 0.72 + (uv.x - 0.5) * 0.86, -1, 1)
  const amount = mode === 'resolve' ? 1 : normalizedStrength * 0.045
  state.targetFinal = mode === 'resolve'
    ? 1
    : clamp(state.targetFinal + amount, 0, 0.86)
}
...
function buildMirrorGroup(THREE, data, series = null, variantDataMap = new Map(), options = {}) {
  ...
  for (const panel of panelDefs) {
    if (!panel.src) continue
    const texture = textureLoader.load(panel.src, loadedTexture => {
    ...
    textures.set(panel.src, texture)
  }
  ...
  const gpuMotionUniforms = enableGpuMotion ? createGpuMotionUniforms(THREE) : null
  const panelStates = panelLayouts.map((layout, panelIndex) => createPanelState(panelIndex, layout))

  const liquidMaterialForPanel = (panel, texture) => {
    if (!texture) return attachGpuMotionToMaterial(innerMaterial.clone(), gpuMotionUniforms)
    if (enableFracture) return createLiquidPosterMaterial(THREE, texture)
    const key = panel.src || panel.id || 'single-panel'
    if (!sharedLiquidMaterials.has(key)) {
      const material = createLiquidPosterMaterial(THREE, texture, gpuMotionUniforms)
      sharedLiquidMaterials.set(key, material)
      liquidPosterUniforms.push(material.userData.liquidUniforms)
    }
  ...
  panelLayouts.forEach((layout, panelIndex) => {
    ...
    for (const shard of panelData.shards) {
      ...
      const shapeGeometry = new THREE.ShapeGeometry(makeShardShape(THREE, localPoints))
      ...
      meshMaterial.opacity = 0.28 + stableNoise(shard.seed, 7) * 0.07
      ...
      const panelMaterial = liquidMaterialForPanel(panel, texture)
      ...
      const edgePoints = localPoints.map(point => new THREE.Vector3(point.x, point.y, 0.006))
...
function targetFromCameraCenter(el, group, detail = {}) {
  ...
  if (hits.length) {
    ...
    return { panelIndex, x: local.x, y: local.y }
  }
  ...
  if (didHitPlane) {
    ...
    const height = group.userData.dimensions?.height || DEFAULT_PHYSICAL_HEIGHT
    return { panelIndex, x: local.x, y: clamp(local.y, 0, height) }
  }
...
export function registerDmsMirrorShards() {
  ...
  window.AFRAME.registerComponent('dms-mirror-shards', {
    schema: {
      ...
      seriesUrl: { default: `${DMS_ROOT}assets/illustration-series.json` },
    ...
    async load() {
      try {
        ...
        console.info(`DMS mirror shards loaded: ${this.group.userData.shardObjects.length} shards across ${this.group.userData.panelCount} panel(s), fracture ${this.group.userData.enableFracture ? 'on' : 'off'}, gpu motion ${this.group.userData.enableGpuMotion ? 'on' : 'off'}`)
      } catch (error) {
    ...
    addInteraction(x, y, strength = 1, panelIndex = 0, liquidMode = 'pulse') {
      ...
      triggerPanelStress(this.group, panelIndex, x, y, strength, t)
      driveLiquidPosterEffect(this.group, x, y, strength, panelIndex, liquidMode)
    ...
    onPulse(event) {
      ...
      this.addInteraction(target.x, target.y, randomButtonStrength(detail), target.panelIndex)
    },
    ...
    onNext() {
      ...
      const target = targetFromCameraCenter(this.el, this.group)
      this.addInteraction(target.x, target.y, randomStrongButtonStrength(), target.panelIndex, 'resolve')
    },
    ...
    onReset() {
      ...
      updateGpuMotionUniforms(this.group, this.interactions, performance.now() / 1000)
      updateDebugState(this.el, this.group, {
        dmsInteractionCount: this.interactionCount,
      })
...
export function wireDmsControls() {
  ...
  const emitInstallationEvent = (eventName, detail = {}) => {
    const installation = document.getElementById('dms-installation')
  ...
  bindInstantButton(pulseButton, () => {
    emitInstallationEvent('dms-pulse')
  })
  bindInstantButton(nextButton, () => {
    emitInstallationEvent('dms-next')
  })
